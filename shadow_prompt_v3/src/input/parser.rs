// Parse hotkey strings like "ctrl+shift+v" into KeyCombo. Ported as-is from v2 — pure logic,
// already proven, no design changes needed here.
...
mod tests {
    ...
    #[test]
    fn parses_four_modifiers() {
        let c = parse("ctrl+shift+alt+x").unwrap();
        assert!(c.ctrl && c.shift && c.alt && !c.win);
        assert_eq!(c.key, Key::KeyX);
    }
}

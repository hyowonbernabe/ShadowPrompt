// Single-instance: kill any prior shadowprompt.exe before taking the named mutex. Re-running
// the daemon always wins. Ported as-is from v2.
...
const MUTEX_NAME: &str = "Global\\ShadowPrompt_v3_SingleInstance";
...
pub fn acquire_single_instance() -> anyhow::Result<InstanceLock> {
    ...
    if err == ERROR_ALREADY_EXISTS {
        anyhow::bail!(
            "named mutex {MUTEX_NAME} still held after kill sweep; another instance is running \
             and we lack permission to terminate it"
        );
    }
    ...
    unsafe {
        ...
        if Process32FirstW(snap, &mut entry).is_ok() {
            loop {
                ...
                if name.eq_ignore_ascii_case("shadowprompt.exe") && entry.th32ProcessID != self_pid {
